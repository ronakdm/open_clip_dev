...
# def raking_ratio_prob(image_features, text_features, clusters, num_raking):
#     marginals = (clusters[0].marginal, clusters[1].marginal)
#     X = clusters[0].clustering(image_features)
#     Y = clusters[1].clustering(text_features)
#     X, Y, cmat, marginals = count_freq(X, Y, marginals)
#     logging.info(f"Batch clusters {len(marginals[0])}, {len(marginals[1])}")
#     pmat = raking_ratio(cmat / len(X), marginals, num_raking)
#     prob = pmat[X, Y] / cmat[X, Y]
#     return prob / np.sum(prob)


def get_raking_ratio_weight(idx, quantization, num_rounds):
    marginals = (quantization["image_marginal"], quantization["text_marginal"])
    X = quantization["image_labels"][idx]
    Y = quantization["text_labels"][idx]
    X, Y, cmat, marginals = count_freq(X, Y, marginals)
    logging.info(f"Batch clusters {len(marginals[0])}, {len(marginals[1])}")
