...
def main(args):
    ...
    if is_master(args, local=args.log_local):
        ...
        if os.path.exists(args.log_path) and not resume_latest and not args.overwrite:
            print(
    ...
    if args.use_raking:
        quantization = {
            key: np.load(os.path.join(args.quantization, f"{key}.npy"))
            for key in [
                "image_labels",
                "text_labels",
                "image_marginal",
...
def copy_codebase(args):
    ...
    if os.path.exists(new_code_path) and not args.overwrite:
        print(
